...
def musdb18_transform(sampling_rate, window_sizes, device, transform_callable, target_path, fnames):
    ...
    transformed_audio_iter = audio_transform(audio_iter(fnames, sampling_rate), window_sizes, device, transform)
    for fname, transformed_audio in zip(fnames, transformed_audio_iter):
        basename = os.path.basename(os.path.splitext(fname)[0])
        target_name = '{}/{}_vocals.wav'.format(target_path, basename)
